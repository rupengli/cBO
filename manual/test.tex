...
For up-to-date installation instructions, use the official OPM page:
\begin{center}
\url{https://opm-project.org/?page_id=36}
\end{center}
In short, OPM recommends using binary packages when available (e.g., Ubuntu via PPA and RHEL via an online repository), and building from source otherwise. The same page links to:
\begin{itemize}
  \item \textbf{Binary Packages}: detailed steps for supported Linux distributions.
  \item \textbf{Compiling from source}: generic Linux/macOS build instructions.
  \item \textbf{Windows}: guidance for running Flow under WSL2 or via a virtual machine.
\end{itemize}
...
\end{center}
After downloading, unzip the dataset and ensure the folder layout matches what the code expects. By default, the config uses:
\[
...
With OPM Flow installed and the data files in place, a typical run is:
\begin{verbatim}
